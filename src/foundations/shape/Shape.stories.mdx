import { ArgsTable, Meta, Story, Canvas, Title, Subtitle } from '@storybook/addon-docs/blocks';
import { DependenciesTable } from 'storybook-addon-deps/blocks';

import Shape from './index';
import Grid, { GridSpacing } from '../layout';
import Typography from '../typography';

export const Template = (args) => <Shape {...args} />;

export const defaultArgs = {
    borderWidth: 1,
    borderRadius: 4,
    elevation: 1,
    elevationOnHover: 2,
    component: 'section',
    width: 200,
    height: 50,
};

export const argTypes = {
    forwardedAs: { table: { disable: true } },
    theme: { table: { disable: true } },
    ref: { table: { disable: true } },
    as: {
        type: { name: 'string', required: false },
        defaultValue: 'div',
        table: {
            type: { summary: 'string' },
            defaultValue: { summary: 'Hello String' },
        },
        control: {
            type: 'select',
            options: ['div', 'span', 'section', 'aside', 'button'],
        },
    },
    borderRadius: {
        control: {
            type: 'select',
            options: [0, 4, 8, 12, 16, 20, 24, 'circle', 'pill'],
        },
    },
    borderWidth: {
        control: {
            type: 'range',
            min: 0,
            max: 10,
            step: 1,
        },
    },
    borderColor: {
        control: 'color',
    },
    elevation: {
        control: {
            type: 'select',
            options: [0, 1, 2, 3, 4, 5, 6],
        },
    },
    elevationOnHover: {
        control: {
            type: 'select',
            options: [0, 1, 2, 3, 4, 5, 6],
        },
    },
    background: {
        control: 'color',
    },
};

<Meta component={Shape} title="Foundations/Shape" />

<Title>Shape</Title>

<Subtitle>
    Shape is another important aspect of the visual style. When shapes are applied consistently, a
    sense of familiarity, trust, and reliability is reinforced.
</Subtitle>

### Core Shapes

There are 4 core shapes used throughout the Mattermost interface:

<Grid padding={GridSpacing.symmetric({ vertical: 150 })} justify={'stretch'}>
    <Grid
        row
        padding={GridSpacing.symmetric({ vertical: 150, horizontal: 25 })}
        alignment={'center'}
        justify={'space-around'}
    >
        <Grid justify={'center'}>
            <Shape {...defaultArgs} borderRadius={0} />
        </Grid>
        <Grid justify={'center'}>
            <Shape {...defaultArgs} borderRadius={12} />
        </Grid>
        <Grid justify={'center'}>
            <Shape {...defaultArgs} borderRadius={'pill'} />
        </Grid>
        <Grid justify={'center'}>
            <Shape {...defaultArgs} borderRadius={'circle'} />
        </Grid>
    </Grid>
</Grid>

## Rectangle

This is often used for larger, more global containers that extend to the edges of the app window.

## Rounded Rectangle

The rounded rectangle is the most widely used shape for elements like Buttons, Modals, or Popover Menus.

### Default Corner Radius

The default corner radius for a container is 4px.
This is used by Buttons, Icon Buttons, Popover Menus, and many other interface elements throughout the product.

export const radiusExampleArgs = {
    ...defaultArgs,
    elevation: 2,
    elevationOnHover: 5,
    width: 120,
    height: 120,
};

<Grid padding={GridSpacing.symmetric({ vertical: 150 })} justify={'stretch'}>
    <Grid row alignment={'center'}>
        <Grid justify={'center'} flex={0}>
            <Grid padding={GridSpacing.all(150)}>
                <Shape {...radiusExampleArgs} width={156} height={156} borderRadius={4} />
            </Grid>
            <br />
            <Typography element={'p'} size={200} margin={'none'}>
                <strong>4px</strong>
            </Typography>
        </Grid>
    </Grid>
</Grid>

###### Other Corner Radiuses

---

<Grid padding={GridSpacing.symmetric({ vertical: 150 })} justify={'stretch'}>
    <Grid row justify={'space-between'}>
        <Grid justify={'center'} flex={0}>
            <Grid padding={GridSpacing.all(150)}>
                <Shape {...radiusExampleArgs} borderRadius={8} />
            </Grid>
            <br />
            <Typography element={'p'} size={200} margin={'none'}>
                <strong>8px</strong>
            </Typography>
        </Grid>
        <Grid justify={'center'} flex={0}>
            <Grid padding={GridSpacing.all(150)}>
                <Shape {...radiusExampleArgs} borderRadius={12} />
            </Grid>
            <br />
            <Typography element={'p'} size={200} margin={'none'}>
                <strong>12px</strong>
            </Typography>
        </Grid>
        <Grid justify={'center'} flex={0}>
            <Grid padding={GridSpacing.all(150)}>
                <Shape {...radiusExampleArgs} borderRadius={16} />
            </Grid>
            <br />
            <Typography element={'p'} size={200} margin={'none'}>
                <strong>16px</strong>
            </Typography>
        </Grid>
        <Grid justify={'center'} flex={0}>
            <Grid padding={GridSpacing.all(150)}>
                <Shape {...radiusExampleArgs} borderRadius={20} />
            </Grid>
            <br />
            <Typography element={'p'} size={200} margin={'none'}>
                <strong>20px</strong>
            </Typography>
        </Grid>
        <Grid justify={'center'} flex={0}>
            <Grid padding={GridSpacing.all(150)}>
                <Shape {...radiusExampleArgs} borderRadius={24} />
            </Grid>
            <br />
            <Typography element={'p'} size={200} margin={'none'}>
                <strong>24px</strong>
            </Typography>
        </Grid>
    </Grid>
</Grid>

## Pill Shape

The pill shape is essentially a Rounded Rectangle with the radius set to half the height of the container.
This shape is used in a few select places—namely the Mention Badge.

## Circle

The circle shape is mostly reserved for the Avatar component.
This helps distinguish users from other UI elements throughout the product.
There are a few other elements that use the circle shape, like the Unread Badge or the Status Badge.

<ArgsTable of={Shape} exclude={['theme', 'forwardedAs', 'ref', 'as']} />

export const canvasExampleArgs = {
    ...defaultArgs,
    width: 400,
    height: 150,
};

## Dependencies

<DependenciesTable titleDependencies="Dependencies" titleDependents="Dependents" of={Shape} />

<Canvas hidden>
    <Story
        name="Rectangle"
        argTypes={argTypes}
        args={{
            ...canvasExampleArgs,
            borderRadius: 0,
            elevation: 0,
        }}
    >
        {Template.bind({})}
    </Story>
    <Story
        name="Rounded Rectangle"
        argTypes={argTypes}
        args={{
            ...canvasExampleArgs,
            borderRadius: 16,
        }}
    >
        {Template.bind({})}
    </Story>
    <Story
        name="Pill"
        argTypes={argTypes}
        args={{
            ...canvasExampleArgs,
            borderRadius: 'pill',
            elevation: 2,
        }}
    >
        {Template.bind({})}
    </Story>
    <Story
        name="Circle"
        argTypes={argTypes}
        args={{
            ...canvasExampleArgs,
            borderRadius: 'circle',
            elevation: 3,
            component: 'section',
            width: 400,
            height: 100,
        }}
    >
        {Template.bind({})}
    </Story>
</Canvas>
